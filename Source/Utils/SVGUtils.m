...
SVGColor SVGColorFromString (const char *string) {
    ...
    if (!strncmp(string, "rgb(", 4) || !strncmp(string, "rgba(", 5)) {
        ...
        int accumIdx = 0, currComponent = 0, handled = 0;
        ...
        for (size_t n = 0; n < len; n++) {
            ...
            if (phase == PhaseRGB || phase == PhaseRGBA) {
                if (c == ',') {
                    ...
                    if (currComponent == 0) {
                        color.r = (uint8_t)lround(atof(accum) * 255.0f / 100.0f);
                        currComponent++;
                    }
                    else if (currComponent == 1) {
                        color.g = (uint8_t)lround(atof(accum) * 255.0f / 100.0f);
                        currComponent++;
                    }
                    else if (currComponent == 2) {
                        color.b = (uint8_t)lround(atof(accum) * 255.0f / 100.0f);
                        currComponent++;
                    }
                    handled = 1;
                    bzero(accum, MAX_ACCUM);
